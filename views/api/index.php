...
<div class="ui r" style="max-width: none !important;">
            ...
            <?php foreach ($routes as $id => $item): ?>
                ...
                <?php foreach($item as $key => $value): ?>
                    <li class="site-tree-noicon <?= $id == '' ? 'layui-this' : '' ?>">
                        <a href="#<?= $value['actionLink'] ?>"><cite><?= $value['title'] ?></cite></a>
                    </li>
                <?php endforeach; ?>
            <?php endforeach; ?>
        </ul>
        ...
        <?php foreach ($routes as $id => $item): ?>
            <div id="<?= $id ?>">
                <h1 class="site-h1"><?= $id ?></h1>
                    <?php foreach($item as $key => $value): ?>
                        <div id="<?= $value['actionLink'] ?>" class="ui raised segment">
                            <span class="ui red ribbon label"><?= $value['title'] ?></span>
                            <div class="ui message">
                                <p>接口链接：<?= $value['actionLink'] ?></p>
    ...
    <!--                                    <th>默认值</th>-->
                                ...
                                <tbody>
                                <?php if($value['param'])foreach ($value['param'] as $param): ?>
                                    <tr>
                                        <td><?= $param['data'] ?></td>
                                        <td><?= $param['type'] ?></td>
    <!--                                        <td>--><?//= $param['require'] ? '是' : '否' ?><!--</td>-->
    <!--                                        <td>--><?//= $param['default'] ?><!--</td>-->
                                        <td><?= str_replace([';','；'],'<br/>',$param['content']) ?></td>
                                    </tr>
                                <?php endforeach; ?>
                                </tbody>
                            </table>
                            <table class="ui blue celled striped table">
                                ...
                                <tbody>
                                <?php if($value['return'])foreach ($value['return'] as $param): ?>
                                    <tr>
                                        <td><?= $param['data'] ?></td>
                                        <td><?= $param['type'] ?></td>
                                        <td><?= $param['content'] ?></td>
